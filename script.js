...
const fields = [
{selector: "#address", fontSize:14, fontWeight:800, multiline:true},
{selector: ".date", fontSize:14, fontWeight:800, isDate:true},
{selector: ".vehicle", fontSize:14, fontWeight:1000},
{selector: ".brent", fontSize:14, fontWeight:800},
{selector: ".bguarantee", fontSize:14, fontWeight:800},
{selector: ".rent", fontSize:14, fontWeight:800, isMoney:true},
{selector: ".guarantee", fontSize:14, fontWeight:800, isMoney:true},
{selector: ".advance", fontSize:14, fontWeight:800, isMoney:true},
{selector: ".amount", fontSize:14, fontWeight:800, isMoney:true},
{selector: ".tip", fontSize:14, fontWeight:800, multiline:true},
{selector: ".weight", fontSize:14, fontWeight:800},
{selector: ".count", fontSize:14, fontWeight:800},
{selector: ".total", fontSize:14, fontWeight:800, isMoney:true},
{selector: ".date1", fontSize:14, fontWeight:800, isDate:true},
{selector: ".time", fontSize:14, fontWeight:800, isTime:true},
{selector: ".day", fontSize:14, fontWeight:800}
];
...
// ===== Responsive Overlay Block =====
const parentWidth = parentRect.width;
const parentHeight = parentRect.height;

overlay.style.position = "absolute";

// Position in percentages relative to parent
overlay.style.top = ((rect.top - parentRect.top + offsets.topOffset) / parentHeight * 100) + "%";
overlay.style.left = ((rect.left - parentRect.left + offsets.leftOffset) / parentWidth * 100) + "%";

// Size in percentages
overlay.style.width = (rect.width / parentWidth * 100) + "%";
overlay.style.height = (rect.height / parentHeight * 100) + "%";

// Font size scaled to viewport width (responsive)
const isPrintMode = window.matchMedia("print").matches;

if (isPrintMode) {
  // Use points for PDF (stable, print-friendly)
  overlay.style.fontSize = f.fontSize + "pt";
} else {
  // Use viewport width for screen responsiveness
  overlay.style.fontSize = (f.fontSize / parentWidth * 100) + "vw";
}
